---
description: Update an ADR status to Accepted and record the previous status.
---

Update the status of the ADR named `$ARGUMENTS` to `Accepted` and record the
...
3. Read the `== Status` section. If the current status is already `Accepted`,
   report it and leave the file unchanged.
4. Replace the status list item with `- Accepted (<today>) by: @<git-user>`.

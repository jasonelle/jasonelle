...
Additionally look for context inside user provided message via $ARGUMENTS

## Steps

1. Run `git status --short` and `git diff --staged` to inspect the staged changes, and read additional context via $ARGUMENTS.
...
- Never run `git commit`, `git add`, or modify any files — print the message and stop.

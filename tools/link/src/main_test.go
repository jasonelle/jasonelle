...
`)
	...
	const xcodeIcon = "PNG-XCODE"
	const androidIcon = "PNG-ANDROID"
	...
	// Icon output as generated by tools/icon into lib/<platform>/assets
	xcodeAssets := filepath.Join(base, "assets", "xcode")
	mkfile(t, filepath.Join(xcodeAssets, "AppIcon.appiconset", "Contents.json"), `{"images":[]}`)
	mkfile(t, filepath.Join(xcodeAssets, "AppIcon.appiconset", "icon-1024.png"), xcodeIcon)
	androidAssets := filepath.Join(base, "assets", "android")
	for _, dpi := range []string{"mipmap-mdpi", "mipmap-xxhdpi"} {
		mkfile(t, filepath.Join(androidAssets, "icon", "res", dpi, "ic_launcher.png"), androidIcon)
		mkfile(t, filepath.Join(androidAssets, "icon", "res", dpi, "ic_launcher_round.png"), androidIcon)
	}
	mkfile(t, filepath.Join(android, "Application", "src", "main", "AndroidManifest.xml"), `<?xml version="1.0" encoding="utf-8"?>
...
`)
	...
	args := []string{"--xcode-sources", xcode, "--android-sources", android, "--xcode-assets", xcodeAssets, "--android-assets", androidAssets}
	...
	// Icons land in Assets.xcassets and src/main/res
	if got := read(t, filepath.Join(xcode, "Application", "Application", "Assets.xcassets", "AppIcon.appiconset", "icon-1024.png")); got != xcodeIcon {
		t.Errorf("Xcode icon not copied: %q", got)
	}
	for _, dpi := range []string{"mipmap-mdpi", "mipmap-xxhdpi"} {
		for _, f := range []string{"ic_launcher.png", "ic_launcher_round.png"} {
			if got := read(t, filepath.Join(android, "Application", "src", "main", "res", dpi, f)); got != androidIcon {
				t.Errorf("Android icon %s/%s not copied: %q", dpi, f, got)
			}
		}
	}
	manifest := read(t, filepath.Join(android, "Application", "src", "main", "AndroidManifest.xml"))
	if !strings.Contains(manifest, `android:icon="@mipmap/ic_launcher"`) || !strings.Contains(manifest, `android:roundIcon="@mipmap/ic_launcher_round"`) {
		t.Errorf("manifest missing icon attrs:\n%s", manifest)
	}

	// A second run must leave every file unchanged
	snapshot := []string{ws, pbx, settings, app}
	snapshot = append(snapshot, copied...)
	snapshot = append(snapshot, manifest,
		read(t, filepath.Join(xcode, "Application", "Application", "Assets.xcassets", "AppIcon.appiconset", "icon-1024.png")),
		read(t, filepath.Join(android, "Application", "src", "main", "res", "mipmap-mdpi", "ic_launcher.png")),
	)
	if err := run(args); err != nil {
		t.Fatal(err)
	}
	current := []string{
		...
		read(t, filepath.Join(android, "Application", "src", "main", "assets", "config.jsonc")),
		read(t, filepath.Join(android, "Application", "src", "main", "assets", "webview.js")),
		read(t, filepath.Join(android, "Application", "src", "main", "AndroidManifest.xml")),
		read(t, filepath.Join(xcode, "Application", "Application", "Assets.xcassets", "AppIcon.appiconset", "icon-1024.png")),
		read(t, filepath.Join(android, "Application", "src", "main", "res", "mipmap-mdpi", "ic_launcher.png")),
	}
...
func TestMirrorDir(t *testing.T) {
	...
	mkfile(t, filepath.Join(src, "AppIcon.appiconset", "Contents.json"), `{"images":[]}`)
	mkfile(t, filepath.Join(src, "AppIcon.appiconset", "icon-1024.png"), "PNG")

	dst := filepath.Join(base, "dst")
	if err := mirrorDir(src, dst); err != nil {
		t.Fatal(err)
	}
	for _, f := range []string{"AppIcon.appiconset/Contents.json", "AppIcon.appiconset/icon-1024.png"} {
		if got := read(t, filepath.Join(dst, f)); got == "" {
			t.Errorf("missing %s", f)
		}
	}

	// Rerun is byte-stable
	snapshot := read(t, filepath.Join(dst, "AppIcon.appiconset", "Contents.json"))
	if err := mirrorDir(src, dst); err != nil {
		t.Fatal(err)
	}
	if again := read(t, filepath.Join(dst, "AppIcon.appiconset", "Contents.json")); again != snapshot {
		t.Errorf("rerun changed Contents.json: %q != %q", again, snapshot)
	}

	// Stale files in dst are removed, new source files land
	mkfile(t, filepath.Join(dst, "AppIcon.appiconset", "stale.png"), "OLD")
	mkfile(t, filepath.Join(src, "AppIcon.appiconset", "icon-180.png"), "PNG2")
	if err := mirrorDir(src, dst); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(filepath.Join(dst, "AppIcon.appiconset", "stale.png")); !os.IsNotExist(err) {
		t.Errorf("stale file not removed: %v", err)
	}
	if got := read(t, filepath.Join(dst, "AppIcon.appiconset", "icon-180.png")); got != "PNG2" {
		t.Errorf("icon-180 not copied: %q", got)
	}

	// Missing source dir fails
	if err := mirrorDir(filepath.Join(base, "nope"), dst); err == nil {
		t.Error("got nil, want missing source error")
	}
}
...
func TestVerifyIcons(t *testing.T) {
	...
	mkfile(t, filepath.Join(src, "AppIcon.appiconset", "icon-1024.png"), "PNG")
	dst := filepath.Join(base, "dst")
	mkfile(t, filepath.Join(dst, "AppIcon.appiconset", "icon-1024.png"), "PNG")
	if err := verifyIcons(src, dst); err != nil {
		t.Fatalf("matching copy failed verify: %v", err)
	}

	// A mismatched byte is caught
	mkfile(t, filepath.Join(dst, "AppIcon.appiconset", "icon-1024.png"), "OTHER")
	if err := verifyIcons(src, dst); err == nil || !strings.Contains(err.Error(), "does not match") {
		t.Errorf("got %v, want mismatch error", err)
	}

	// A missing file is caught
	if err := os.Remove(filepath.Join(dst, "AppIcon.appiconset", "icon-1024.png")); err != nil {
		t.Fatal(err)
	}
	if err := verifyIcons(src, dst); err == nil || !strings.Contains(err.Error(), "icon-1024.png") {
		t.Errorf("got %v, want missing file error", err)
	}
}

func TestEnsureManifestIcons(t *testing.T) {
	noAttr := `<?xml version="1.0" encoding="utf-8"?>
<manifest xmlns:android="http://schemas.android.com/apk/res/android">
    <application
        android:label="Jasonelle"
        android:theme="@android:style/Theme.Material.Light.NoActionBar">
...
`
	got, err := ensureManifestIcons(noAttr)
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{
	...
	// Reruns are byte-stable
	if again, err := ensureManifestIcons(got); err != nil || again != got {
		t.Errorf("rerun not stable (err=%v):\n%s", err, again)
	}

	// Existing attributes have their values replaced, everything else kept
	hasAttr := `<?xml version="1.0" encoding="utf-8"?>
<manifest xmlns:android="http://schemas.android.com/apk/res/android">
    <application
        android:icon="@drawable/old"
        android:label="Jasonelle"
        android:roundIcon="@drawable/old_round">
        <activity android:name=".MainActivity" />
    </application>
</manifest>
`
	got, err = ensureManifestIcons(hasAttr)
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(got, `android:icon="@mipmap/ic_launcher"`) {
		t.Errorf("icon not replaced:\n%s", got)
	}
	if !strings.Contains(got, `android:roundIcon="@mipmap/ic_launcher_round"`) {
		t.Errorf("roundIcon not replaced:\n%s", got)
	}
	if strings.Contains(got, "@drawable/old") {
		t.Errorf("stale icon value kept:\n%s", got)
	}

	// Missing <application> fails loudly
	if _, err := ensureManifestIcons(`<manifest></manifest>`); err == nil || !strings.Contains(err.Error(), "<application>") {
		t.Errorf("got %v, want missing application error", err)
	}
}

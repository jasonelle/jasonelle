...
| `Plugin.kt`
| Native side – saves and reads the cookie string from `EncryptedSharedPreferences`.
...
* `Plugin.js` in `src/main/assets/plugins/cookies/` – the JavaScript client, byte-identical to the xref:xcode:plugin-cookies.adoc[Xcode plugin].
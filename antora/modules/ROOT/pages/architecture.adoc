...
5. *Unified Build Pipeline*: the `gen` tool (run via the `Taskfile` `gen` task)
   ...
   `link` and `appid` to assemble both `build/xcode` and `build/android` from
...
System_Boundary(jasonelle, "Jasonelle Framework") {
    Container(gen, "gen", "Go", "Orchestrates the pipeline: icon, jsonc, bundler, plugins, core, link and appid")
...
`plugins`, `core`, `link`, `appid`) as the `Taskfile` would (Go).

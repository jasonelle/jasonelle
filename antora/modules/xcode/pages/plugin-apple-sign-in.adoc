...
| `Plugin.swift`
| Native iOS side – bridges to `AuthenticationServices` (`ASAuthorizationController`, `ASAuthorizationAppleIDProvider`).

| `Plugin.js`
| JavaScript side – registers on `window.jasonelle.plugins.applesignin`, provides the Button Controller and HIG renderer.
|===

...
(() => {
  ...
  // Store a cookie string in the encrypted storage
  // Example: window.jasonelle.plugins.cookies.save("session=abc; Path=/")
  plugin.save = (value) => native.post(plugin.id, { action: "save", value });

  // Restore the cookie string from the encrypted storage and inject it into the webview
  ...
  // Store the current webview cookies in the encrypted storage

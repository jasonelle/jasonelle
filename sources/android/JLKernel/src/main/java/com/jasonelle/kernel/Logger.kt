 ...
 */
enum class LogLevel(val value: Int) : Comparable<LogLevel> {
    DEBUG(0),
    INFO(1),
    NOTICE(2),
    WARNING(3),
...
object Ratlog {
    @JvmStatic
    fun format(message: String, tags: List<String> = emptyList(), fields: Map<String, String> = emptyMap()): String {
        ...
        if (fields.isNotEmpty()) {
            for (key in fields.keys.sorted()) {
                sb.append(" | ").append(key).append(": ").append(fields[key] ?: "")
            }
        }
...
class Logger {
    ...
    fun debug(message: String, tags: List<String> = emptyList(), fields: Map<String, String> = emptyMap()) {
        log(LogLevel.DEBUG, "debug", message, tags, fields)
    }

    fun info(message: String, tags: List<String> = emptyList(), fields: Map<String, String> = emptyMap()) {
        log(LogLevel.INFO, "info", message, tags, fields)
    }

    fun notice(message: String, tags: List<String> = emptyList(), fields: Map<String, String> = emptyMap()) {
        log(LogLevel.NOTICE, "notice", message, tags, fields)
    }

    fun warning(message: String, tags: List<String> = emptyList(), fields: Map<String, String> = emptyMap()) {
        log(LogLevel.WARNING, "warning", message, tags, fields)
    }

    fun error(message: String, tags: List<String> = emptyList(), fields: Map<String, String> = emptyMap()) {
        log(LogLevel.ERROR, "error", message, tags, fields)
    }

    fun critical(message: String, tags: List<String> = emptyList(), fields: Map<String, String> = emptyMap()) {
        log(LogLevel.CRITICAL, "critical", message, tags, fields)
    }

    fun emergency(message: String, tags: List<String> = emptyList(), fields: Map<String, String> = emptyMap()) {

...
class Coordinator(
     ...
     */
    fun handleMessage(name: String, callbackId: String, args: Map<String, Any?>) {
     ...
     */
    fun decidePolicy(url: Uri?, allowed: List<String>?, mainURL: Uri?): NavigationPolicy {
        return decidePolicyForHost(url?.host, allowed, mainURL?.host)
    }

    /**
     * Host-only policy decision, extracted for JVM unit testing.
     */
    fun decidePolicyForHost(host: String?, allowed: List<String>?, mainHost: String?): NavigationPolicy {
    ...
    fun openInCustomTabs(context: Context, url: Uri) {
        try {
...
fun injectUserScripts(
    ...
    val appScript = appScriptSource ?: try {
        context.assets.open("webview.js").bufferedReader().use { it.readText() }
    } catch (_: Exception) {
        null
    }

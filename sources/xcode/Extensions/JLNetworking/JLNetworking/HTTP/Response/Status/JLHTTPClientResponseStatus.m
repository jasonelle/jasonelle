...
#import "JLHTTPClientResponseStatus.h"

@implementation JLHTTPClientResponseStatus

- (instancetype) initWithCode:(JLHTTPClientResponseStatusCode) code {
    self = [super init];
    _code = code;
    _name = [JLHTTPClientResponseStatus nameForCode:code];
    _about = [JLHTTPClientResponseStatus descriptionForCode:code];
    return self;
}

- (NSString *) description {
    return [NSString stringWithFormat:@"(%lu) %@ | %@", (unsigned long)self.code, self.name, self.about];
}
